...
<body>
    ...
    <?php
        $listCategory = mysqli_query($conn, "SELECT * FROM category WHERE id_parent = 0 ORDER BY id_category DESC LIMIT 3");
            if(mysqli_num_rows($listCategory) > 0){
                while($oneCate = mysqli_fetch_assoc($listCategory)){
    ?>
    <section id="<?=$oneCate['name']?>">
        <div class="container px-5 mx-auto">
    ...
    <section class="show-products">
        <div class="box-container">
            <?php
                    $select_book = mysqli_query($conn, "SELECT * FROM `book_info` where id_category=".$oneCate['id_category']." LIMIT 4") or die('query failed');
                    if (mysqli_num_rows($select_book) > 0) {
                        while ($fetch_book = mysqli_fetch_assoc($select_book)) {
            ?>
                    <div class="box card-book">
